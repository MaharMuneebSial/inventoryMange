...
export default function SalesList() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      // F2: Jump to first table row
      if (e.key === 'F2') {
        e.preventDefault();
        if (currentItems.length > 0) {
          setSelectedRow(0);
          const firstViewButton = document.querySelector('.view-btn');
          if (firstViewButton) firstViewButton.focus();
        }
        return;
      }
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (currentItems.length > 0) {
          setSelectedRow((prev) => {
            ...
            if (viewButtons[newRow]) {
              viewButtons[newRow].focus();
            }
            return newRow;
          });
        }
      ...
      if (e.key === 'Enter') {
        e.preventDefault();
        if (currentItems.length > 0 && currentItems[selectedRow]) {
  ...
  }, [selectedRow, currentItems, showDetailsModal, totalPages]);

  // Calculate totals
  const totalGrossSales = sales.reduce((sum, sale) => sum + parseFloat(sale.grand_total || 0), 0);
  const totalReturns = sales.reduce((sum, sale) => sum + parseFloat(sale.total_returned || 0), 0);
  const totalNetSales = sales.reduce((sum, sale) => {
    const netTotal = sale.net_total || (parseFloat(sale.grand_total || 0) - parseFloat(sale.total_returned || 0));
    return sum + netTotal;
  }, 0);
    ...
    <div className="h-full flex flex-col gap-3">
      ...
      {showDetailsModal && selectedSale && (
        <div className="fixed inset-0 backdrop-blur-md flex items-center justify-center z-50 p-4" style={{ backgroundColor: 'rgba(0, 0, 0, 0.4)' }}>
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full border border-gray-100">
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-teal-600 to-cyan-600 text-white px-4 py-3 rounded-t-lg">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={closeModal}
                  className="text-white hover:text-gray-200 transition-colors focus:ring-2 focus:ring-white focus:outline-none rounded"
                  title="Close (Esc)"
                >
            ...
            <div className="bg-gray-50 px-4 py-3 rounded-b-lg flex justify-end">
              <button
                ref={closeButtonRef}
                onClick={closeModal}
                className="px-3 py-1.5 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition text-xs font-medium focus:ring-2 focus:ring-gray-500 focus:outline-none"
              >
                Close
              </button>
            </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          onClick={loadSales}
          className="px-3 py-1.5 bg-teal-600 text-white rounded-md hover:bg-teal-700 transition flex items-center gap-1.5 text-xs font-medium focus:ring-2 focus:ring-teal-500 focus:outline-none"
        >
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  ...
                  currentItems.map((sale, index) => (
                    <tr key={sale.id} className={`hover:bg-blue-50 transition-colors ${sale.has_returns ? 'bg-orange-50' : ''} ${
                      selectedRow === index ? 'ring-2 ring-teal-400 bg-teal-50' : ''
                    }`}>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">
                        <span className="text-xs font-bold text-green-600">Rs. {parseFloat(sale.net_total || (sale.grand_total - (sale.total_returned || 0))).toFixed(2)}</span>
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-xs font-medium">
                        <button
                          ...
                          className="view-btn p-1 text-blue-600 hover:bg-blue-100 rounded transition-all focus:ring-2 focus:ring-blue-500 focus:outline-none"
                          title="View"
                        >
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
